...
The single public export is `Game` (`chesssnake/__init__.py` → `from .remote.Game import Game`). It is a `chesslib.Game` subclass that is **local by default** and **remote when asked**:
...
1. **Client** (`chesssnake/remote/`) — `Game`/`Challenge` (`Game.py`) run the engine locally; `ApiClient` (`client.py`) is a thin `requests`-style wrapper. `requests` is imported lazily, only on the `remote=True` path, so local games and the engine stay dependency-free.
2. **Server** (`chesssnake/api/server.py`) — a FastAPI `app` exposing a thin persistence API (get-or-create, update, draw patch/clear, delete, current/exists, challenges, `/health`). It **never imports `chesslib`** — it only moves serialized strings/ids in and out of Postgres. Domain errors are mapped to JSON `{error_type, detail}` with status codes (id→422, challenge→409, sql→500); the client re-raises the matching `GameError` type.
3. **Database** (`chesssnake/postgres/`) — the SQL. `operations.py` holds the pure query functions the server calls; `PSql_Utils.py` the pool + `execute_psql`; `GameError.py` the exception types (now `Exception`-based so FastAPI can catch them); `data/init.sql` the schema.

`chesssnake/cli.py` is the `chesssnake` console-script entry point (`api-endpoint`, `init-db` subcommands).

When changing **gameplay** behavior, edit `chesslib` — every tier inherits it. When changing **persistence**, edit `postgres/operations.py` (SQL) and mirror the endpoint in `api/server.py` + the method in `remote/client.py`.
...
- `operations.py` — the pure SQL operations the server calls (`game_get_or_create`, `game_update`, `game_update_draw/clear`, `game_delete`, `current_games`, `game_exists`, `challenge*`, `db_init`). They deal only in primitives/dicts — **no `chesslib` import** — and validate ids via `validate_ids`.
...
- `GameError.py` — SQL/challenge exception hierarchy rooted at `GameError` (an `Exception` subclass, separate from `chesslib`'s `ChessError`; shared by client and server for error mapping).
