...
- `Game.remote(white_id, black_id, group_id=..., player_id=..., api_url=..., api_key=..., client=...)` — a game persisted through the api-endpoint. **The server runs the engine**: `move`/`draw_*`/`resign` send a request, and the returned state is mirrored into the local board (for rendering + accessors). Illegal moves raise the same `ChessError` types as local play. `refresh()` re-fetches server state. `player_id` (if given) is asserted server-side, and the client auto-sends its `version` for optimistic concurrency.
...
The wire payloads are defined once in `src/chesssnake/dto.py`: `GameState` (the position as **FEN** + `status`/`version`/`draw`/`termination`/names) and `MoveResult` (a move + resulting state) — stdlib dataclasses, so the client needs no pydantic. The FEN codec is `src/chesssnake/engine/fen.py` (`to_fen`/`from_fen`/`position_key`/`INITIAL_FEN`), and `src/chesssnake/serialize.py` bridges `GameState` ⇄ engine `Game` (`game_from_state` with history, `state_from_game`, `board_and_turn`). `src/chesssnake/cli.py` is the console-script entry point; `src/chesssnake/assets.py` (`asset_path`) centralizes packaged-data lookups.
...
- `image.py` — `render_board(board, white_name, black_name, move=None) -> PIL.Image`; composites cached piece PNGs, both orientations, last-move highlight. 68px tiles.
...
- `postgres.py` — the PostgreSQL backend: `game_get_or_create` (inserts the FEN game + a ply-0 `Moves` row), `game_get`, `game_history`, `apply_game_change` (`SELECT…FOR UPDATE` game + `Moves` history → `mutate(row, history)` callback → `UPDATE` + bump `Version` + append `Moves`, all in one `transaction()`; enforces `expected_version`), `game_delete` (game + its moves), `current_games`, `game_exists`, `challenge*`, `db_init`. Deals only in primitives/dicts — **no `engine` import**; the chess logic is the server's `mutate` callback. The engine-derived initial FEN/position key are passed *in* to `game_get_or_create`. Validates ids via `validate_ids`.
...
- `data/init.sql` — idempotent schema for `Games`, `Moves`, and `Challenges`. `Games` PK `(GroupId, WhiteId, BlackId)` stores `Fen TEXT`, `Draw`/`Status` (INTEGER; Status 0–3), `Termination TEXT`, `Version INTEGER`, names, and an `UpdatedAt` trigger. `Moves` PK `(GroupId, WhiteId, BlackId, Ply)` stores `San` + `PositionKey`. There is no `Groups` table — `GroupId` is just a discriminator, not a foreign key.
